...
import {clampValueToRange, getEmpty2dArray, stdDev} from "../../util";
...
import {PatternProvider} from "./pattern_provider";
...
export class NoteGenerator implements AccuracyObserver, Drawable {
    ...
    private hardStdDev: number;

    public constructor(noteManager: NoteManager) {
        this.noteManager = noteManager;
        this.numTracks = this.noteManager.tracks.length;
        ...
        this.scoreMemory = [];
        this.accuracyMemory = [];
        this.numTracks = global.experimentConfig.numTracks;
        ...
        this.patternProvider = new PatternProvider(this.numTracks, global.experimentConfig.chordSizeSequence);
        this.hardStdDev = global.experimentConfig.hardStdDev;
    }
    ...
    public update(accuracyEvent: AccuracyEvent): void {
        if (!AccuracyUtil.eventIsABoo(accuracyEvent, this.config)) {
            ...
            let targetNps = this.inverseModel(this.hardStdDev);
    ...
    public draw(currentTimeInSeconds: number): void {
        ...
        if (this.lastNoteTimeInSeconds - currentTimeInSeconds < 2) {
            let nextNoteTracks: number[] = this.patternProvider.getNextNoteTracks(this.lastNoteTracks);
            for (let i = 0; i < nextNoteTracks.length; i++) {
                let nextNoteTrack = nextNoteTracks[i];
            ...
            this.lastNoteTracks = nextNoteTracks;
